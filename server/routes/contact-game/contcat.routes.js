...
const contactGame = require("../../models/contact-game/Contact.model");
...
router.post(
  ...
  async (req, res) => {
    const { givenName, familyName, mobile, email, subscribe } = req.body;
    ...
    try {
      let contact = await contactGame.findOne({ email });
      if (contact) {
        return res.status(400).json({
          errors: [{ msg: "L'adresse e-mail existe déjà!", param: "email" }],
        });
      }
      contact = new contactGame({
        givenName,
        familyName,
        mobile,
        email,
...
router.route("/:id").delete((req, res, next) => {
  contactGame.findByIdAndRemove(req.params.id, (error, data) => {
    if (error) {
      return next(error);
    } else {
      res.status(200).json({
...
router.route("/:id").get((req, res) => {
  contactGame.findById(req.params.id, (error, data) => {
    if (error) {
      return next(error);
    } else {
      res.json(data);
